...
def nice_box(width: int, contents: Optional[List[Tuple[str, str]]] = None, margin: int = 2, thick: bool = False,
             upper: bool = True, lower: bool = True, sides: bool = True) -> str:
    '''
    Creates a decorative text box with the following options:
    ...
    for content in contents:
        if not isinstance(content, tuple) or len(content) != 2 or not isinstance(content[0], str) or not isinstance(content[1], str):
            raise ValueError(
